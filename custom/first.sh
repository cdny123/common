#!/bin/bash
# https://github.com/cdny/build-actions
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/cdny/common build/common
...
git clone -b main --depth 1 https://github.com/cdny/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (cdny/build-actions|cdny/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/cdny/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/cdny/common/main/common.sh -O common.sh
fi
